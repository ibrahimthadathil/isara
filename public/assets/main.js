/** --- SAFE CLOSEST HELPER (injected) --- **/

function init() {
  function safeClosest(target, selector) {
    try {
      if (!target) return null;
      if (target instanceof Event && target.target) target = target.target;
  ...
  class ModalAnimation {
    ...
    bindEvents() {
      var _a, _b, _c;
      (_a = this.modalAction) == null
        ? void 0
        : _a.addEventListener("click", () => this.openModal());
      (_b = this.modalCloseBtn) == null
        ? void 0
        : _b.addEventListener("click", () => this.closeModal());
      (_c = this.modalOverlay) == null
        ? void 0
    ...
    init() {
      ...
      const filterArticles = (articles, filterCategory) => {
        ...
        articles.forEach((article) => {
          ...
          const shouldShow =
            ...
            (filterCategory === "ai software" &&
              articleCategory === "ai software");
          if (shouldShow) {
      ...
      const animateFilter = async (filtered, hidden) => {
        ...
        const allContainers = allArticles
          ...
          .filter(Boolean);
        const canAnimate =
          typeof gsap !== "undefined" && gsap && typeof gsap.to === "function";
        if (!canAnimate) {
  ...
  const commonAnimation = {
    init() {
      ...
      if (splitTextTeamTitle) {
        ...
        let split = SplitText.create(".split-text-team-title", {
          type: "chars",
        });
  ...
  class NavigationMenu {
    ...
    bindEvents() {
      ...
      document.addEventListener("click", (e) => {
        if (
          !safeClosest(e.target, ".nav-item") &&
          !safeClosest(e.target, ".mega-menu, .dropdown-menu")
        ) {
          this.hideAllMenus();
        }
      });
      ...
      document.addEventListener(
        ...
        (e) => {
          if (
            safeClosest(
              e.target,
              ".mega-menu, .dropdown-menu, .mega-menu-bridge, .dropdown-menu-bridge"
            )
          ) {
    ...
    showMenu(navItem, menu) {
      ...
      const bridge = navItem.querySelector(
  ...
  const progressAnimation = {
    init() {
      ...
      items.forEach((item, index) => {
        const value =
          parseInt(item.getAttribute("data-progress-value"), 10) || 0;
...
console.log("time started 0");
setTimeout(() => {
  console.log("time started 1");
  init();
  console.log("time started 1 csalled");
}, 1300);